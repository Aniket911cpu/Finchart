...
export default function SettingsPage() {
    ...
    return (<div className="flex-1 overflow-y-auto bg-bg-primary text-text-primary min-h-screen">
      <div className="max-w-5xl mx-auto p-6 md:p-10">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-64 flex-shrink-0 space-y-1">
            {tabs.map(tab => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition-colors ${activeTab === tab.id ? 'bg-bg-tertiary text-text-primary' : 'text-text-secondary hover:bg-bg-secondary hover:text-text-primary'}`}>
                  <Icon size={18}/>
                  <span>{tab.label}</span>
                </button>);
        })}
          ...
          <div className="flex-1 bg-bg-secondary border border-border rounded-2xl p-6 md:p-8">
            ...
            {activeTab === 'apikeys' && (<div className="space-y-6">
                ...
                {apiKeys.length === 0 ? (<div className="p-8 text-center border border-dashed border-border rounded-xl text-text-secondary">
                    No API keys generated yet.
                  </div>) : (<div className="space-y-3">
                    {apiKeys.map((key) => (<div key={key.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-border rounded-xl bg-bg-tertiary/30 gap-4">
                        <div>
            ...
            {activeTab === 'appearance' && (<div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" value={settings.defaultSymbol} onChange={e => setSettings({ ...settings, defaultSymbol: e.target.value })} className="w-full bg-bg-tertiary border border-border rounded-lg px-4 py-2 focus:outline-none focus:border-accent-blue uppercase"/>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-text-secondary">Default Timeframe</label>
                    <select value={settings.defaultTimeframe} onChange={e => setSettings({ ...settings, defaultTimeframe: e.target.value })} className="w-full bg-bg-tertiary border border-border rounded-lg px-4 py-2 focus:outline-none focus:border-accent-blue">
            ...
            <div className="mt-8 pt-6 border-t border-border flex justify-end">
              <button onClick={saveSettings} className="flex items-center space-x-2 px-6 py-2.5 bg-accent-blue hover:bg-accent-blue-hover text-white rounded-lg font-medium transition-colors shadow-lg shadow-accent-blue/20">
                <Save size={18}/>
