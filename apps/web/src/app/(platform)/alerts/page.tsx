...
import { Bell, Plus, Filter, Search, CheckCircle2, XCircle, Trash2 } from 'lucide-react';
...
export default function AlertsPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-bg-primary p-6 md:p-10 text-text-primary min-h-screen">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 bg-bg-secondary p-2 border border-border rounded-lg">
          <div className="flex space-x-1 w-full sm:w-auto">
            {['ALL', 'ACTIVE', 'TRIGGERED'].map(f => (
              <button 
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors flex-1 sm:flex-none ${filter === f ? 'bg-bg-tertiary text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
              >
                {f}
              </button>
            ))}
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              className="w-full bg-bg-tertiary border border-border rounded-md pl-9 pr-3 py-1.5 text-sm focus:outline-none focus:border-accent-blue"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {filteredAlerts.length === 0 ? (
                ...
                filteredAlerts.map(alert => (
                  <tr key={alert.id} className="hover:bg-bg-tertiary/50 transition-colors">
                    ...
                    <td className="p-4">
                      {alert.triggered ? (
                        ...
                        <span className="inline-flex items-center space-x-1 text-positive bg-positive/10 px-2 py-0.5 rounded text-xs font-medium">
                    ...
                    <td className="p-4 text-right">
                      <button 
                        ...
                        className="text-text-muted hover:text-negative p-1.5 rounded-md hover:bg-negative/10 transition-colors"
                      >
                        <Trash2 size={16} />
                      </button>
                    </td>
