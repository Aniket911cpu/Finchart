...
import { Activity, TrendingUp, Bell, Layers, ArrowRight, Zap, Brain, Shield, Globe, BarChart2, Users, ChevronRight, Check } from 'lucide-react';
...
export default function LandingPage() {
    ...
    return (<div className="min-h-screen bg-[#0A0D14] text-[#E8E9EC] selection:bg-accent-blue/30 overflow-x-hidden">
      ...
      <header className="h-16 border-b border-white/5 flex items-center justify-between px-6 lg:px-12 backdrop-blur-md sticky top-0 z-50 bg-[#0A0D14]/80">
        ...
        <div className="flex items-center space-x-3">
          {session ? (<Link href="/terminal" className="px-4 py-2 bg-accent-blue hover:bg-accent-blue-hover text-white text-sm font-medium rounded-lg transition-all hover:scale-105 shadow-[0_0_15px_rgba(41,98,255,0.3)]">
              Launch App
            </Link>) : (<>
      ...
      <section className="relative pt-28 pb-16 lg:pt-40 lg:pb-24 px-6 lg:px-12 flex flex-col items-center text-center overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-8 w-full max-w-2xl">
            {STATS.map(({ value, label }) => (<div key={label} className="flex flex-col items-center">
      ...
      <section id="features" className="py-24 px-6 lg:px-12 max-w-7xl mx-auto border-t border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map(({ icon: Icon, title, desc, color, bg }) => (<div key={title} className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] hover:border-white/10 transition-all group">
      ...
      <section className="py-24 px-6 lg:px-12 max-w-5xl mx-auto border-t border-white/5">
        ...
        ].map(([feat, tv, fp]) => (<tr key={String(feat)} className="border-b border-white/5 hover:bg-white/[0.02]">
      ...
      <section id="pricing" className="py-24 px-6 lg:px-12 max-w-6xl mx-auto border-t border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PLANS.map((plan) => (<div key={plan.name} className={`relative flex flex-col rounded-2xl p-6 border transition-all ${plan.highlight
                ? 'bg-accent-blue/10 border-accent-blue/40 shadow-[0_0_60px_rgba(41,98,255,0.15)]'
                : 'bg-white/[0.02] border-white/[0.06]'}`}>
              {plan.badge && (<div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-accent-blue text-white text-xs font-semibold rounded-full">
                  {plan.badge}
                </div>)}
              <div className="mb-6">
              ...
              <ul className="space-y-2.5 mb-8 flex-1">
                {plan.features.map((f) => (<li key={f} className="flex items-start space-x-2.5 text-sm">
                    <Check size={14} className={`flex-shrink-0 mt-0.5 ${plan.highlight ? 'text-accent-blue' : 'text-positive'}`}/>
                    <span className="text-text-secondary">{f}</span>
                  </li>))}
              </ul>

              <Link href={plan.href} className={`w-full py-3 rounded-xl text-sm font-semibold text-center transition-all ${plan.highlight
