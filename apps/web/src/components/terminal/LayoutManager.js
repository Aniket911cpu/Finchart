...
export function LayoutManager() {
    ...
    return (<div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center space-x-2 px-3 py-1.5 bg-bg-tertiary rounded-md hover:bg-bg-tertiary/80 transition-colors text-text-primary">
      ...
      {isOpen && (<>
          ...
          <div className="absolute top-full mt-2 right-0 w-56 bg-bg-secondary border border-border rounded-lg shadow-xl z-50 overflow-hidden py-1">
            {!session ? (<div className="p-4 text-xs text-text-secondary text-center">
