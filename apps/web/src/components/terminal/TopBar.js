    ...
    return (<>
      <div className="h-14 border-b border-border bg-bg-secondary flex items-center justify-between px-4 shrink-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button onClick={() => useChartStore.getState().setAlertsOpen(!useChartStore.getState().isAlertsOpen)} className={`p-1.5 rounded transition-colors ${useChartStore.getState().isAlertsOpen ? 'bg-accent-blue/20 text-accent-blue' : 'text-text-secondary hover:text-text-primary hover:bg-bg-tertiary'}`} title="Price Alerts">
            <Bell size={18}/>
          </button>
          ...
          {session ? (<div className="flex items-center space-x-3 cursor-pointer group relative">
              <div className="w-8 h-8 rounded-full bg-accent-blue flex items-center justify-center text-white font-bold overflow-hidden">
                {session.user?.image ? (<img src={session.user.image} alt="Avatar" className="w-full h-full object-cover"/>) : (<span>{session.user?.name?.[0] || session.user?.email?.[0]?.toUpperCase()}</span>)}
              </div>
              ...
              <div className="absolute top-full right-0 mt-2 hidden group-hover:block bg-bg-secondary border border-border rounded-lg shadow-xl py-1 w-48 z-50">
                ...
                <button onClick={() => signOut()} className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-bg-tertiary transition-colors">
                  Sign out
                </button>
              </div>
            </div>) : (<button onClick={() => setLoginModalOpen(true)} className="flex items-center space-x-2 px-4 py-1.5 bg-accent-blue/10 hover:bg-accent-blue/20 text-accent-blue font-medium rounded-md transition-colors">
              <User size={16}/>
              <span>Sign In</span>
            </button>)}
        </div>
