...
export function WatchlistSidebar() {
    ...
    if (!isOpen) {
        return (<div className="w-8 border-l border-border bg-bg-secondary flex flex-col items-center py-4 cursor-pointer hover:bg-bg-tertiary transition-colors" onClick={() => setIsOpen(true)}>
        <div className="rotate-90 whitespace-nowrap text-text-secondary font-medium tracking-widest text-xs mt-10">
          WATCHLIST
        </div>
      </div>);
    }
    return (<div className="w-64 border-l border-border bg-bg-secondary flex flex-col h-full transition-all duration-300 relative">
      ...
      <div className="flex-1 overflow-y-auto">
        {!session ? (<div className="p-4 text-center text-text-secondary text-sm mt-10">
            Sign in to create custom watchlists and sync them across devices.
          </div>) : (<div className="p-2 space-y-4">
            {watchlists.map((wl) => (<div key={wl.id} className="space-y-1">
                ...
                {wl.symbols.map((sym) => (<div key={sym.id} onClick={() => setSymbol(sym.symbol)} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-bg-tertiary cursor-pointer group transition-colors">
...
//# sourceMappingURL=WatchlistSidebar.js.map