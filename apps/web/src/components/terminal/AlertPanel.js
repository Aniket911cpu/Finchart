...
export function AlertPanel({ onClose }) {
    ...
    if (!session) {
        return (<div className="w-80 h-full bg-bg-secondary border-l border-border flex flex-col items-center justify-center p-4">
    ...
    return (<div className="w-80 h-full bg-bg-secondary border-l border-border flex flex-col shrink-0">
      ...
      <div className="p-4 border-b border-border bg-bg-tertiary/30">
        ...
        <div className="space-y-3">
          <div className="flex bg-bg-primary rounded-md p-1">
            {['ABOVE', 'BELOW', 'CROSS'].map(c => (<button key={c} onClick={() => setCondition(c)} className={`flex-1 text-xs py-1 rounded transition-colors ${condition === c ? 'bg-accent-blue text-white' : 'text-text-secondary hover:text-text-primary'}`}>
                {c}
              </button>))}
          </div>

          <div className="flex space-x-2">
            <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="Price" className="flex-1 bg-bg-primary border border-border rounded px-3 py-1.5 text-sm text-text-primary focus:outline-none focus:border-accent-blue"/>
            <button onClick={handleCreate} disabled={createAlert.isPending || !price} className="bg-accent-blue hover:bg-accent-blue-hover text-white px-3 py-1.5 rounded flex items-center justify-center disabled:opacity-50">
              <Plus size={16}/>
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isLoading ? (<div className="text-center p-4 text-text-muted text-sm">Loading alerts...</div>) : alerts?.length === 0 ? (<div className="text-center p-8 text-text-muted text-sm flex flex-col items-center">
            <Bell size={32} className="mb-2 opacity-50"/>
            No active alerts
          </div>) : (<div className="space-y-2">
            {alerts?.map(alert => (<div key={alert.id} className="bg-bg-primary border border-border rounded p-3 flex items-center justify-between group">
                <div>
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="font-medium text-sm text-text-primary">{alert.symbol}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${alert.condition === 'ABOVE' ? 'bg-positive/20 text-positive' :
                ...
                <button onClick={() => deleteAlert.mutate(alert.id)} className="text-text-muted hover:text-negative opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-bg-secondary" title="Delete Alert">
                  <Trash2 size={16}/>
                </button>
              </div>))}
          </div>)}
