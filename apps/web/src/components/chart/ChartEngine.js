...
export default function ChartEngine() {
    ...
    useEffect(() => {
        if (!historicalData || !mainSeriesRef.current || !volumeSeriesRef.current || !chartRef.current)
            return;
        ...
        const formattedData = historicalData.map(d => ({
            time: d.time, open: d.open, high: d.high, low: d.low, close: d.close, value: d.close
        }));
        mainSeriesRef.current.setData(formattedData);
        const volData = historicalData.map(d => ({
            time: d.time, value: d.volume, color: d.close >= d.open ? upColor : downColor
        }));
        ...
        const toSeriesData = (values) => {
            return values.map((val, i) => ({ time: times[i], value: val })).filter(d => d.value !== null);
        };
        // --- OVERLAYS ---
        if (indicatorsState.sma20) {
            if (!indicatorsRef.current.sma)
                indicatorsRef.current.sma = chart.addLineSeries({ color: '#FFB300', lineWidth: 2 });
            indicatorsRef.current.sma.setData(toSeriesData(sma(closes, 20)));
        }
        else {
            if (indicatorsRef.current.sma) {
                chart.removeSeries(indicatorsRef.current.sma);
                delete indicatorsRef.current.sma;
            }
        }
        if (indicatorsState.ema9) {
            if (!indicatorsRef.current.ema9)
                indicatorsRef.current.ema9 = chart.addLineSeries({ color: '#2196F3', lineWidth: 2 });
            indicatorsRef.current.ema9.setData(toSeriesData(ema(closes, 9)));
        }
        else {
            if (indicatorsRef.current.ema9) {
                chart.removeSeries(indicatorsRef.current.ema9);
                delete indicatorsRef.current.ema9;
            }
        }
        if (indicatorsState.ema21) {
            if (!indicatorsRef.current.ema21)
                indicatorsRef.current.ema21 = chart.addLineSeries({ color: '#E91E63', lineWidth: 2 });
            indicatorsRef.current.ema21.setData(toSeriesData(ema(closes, 21)));
        }
        else {
            if (indicatorsRef.current.ema21) {
                chart.removeSeries(indicatorsRef.current.ema21);
                delete indicatorsRef.current.ema21;
            }
        }
        if (indicatorsState.bb20) {
            ...
            if (indicatorsRef.current.bbUpper) {
                ...
                delete indicatorsRef.current.bbUpper;
                delete indicatorsRef.current.bbLower;
            }
        }
        if (indicatorsState.vwap) {
            if (!indicatorsRef.current.vwap)
                indicatorsRef.current.vwap = chart.addLineSeries({ color: '#9C27B0', lineWidth: 2 });
            const vwapData = vwap(historicalData.map(d => ({ high: d.high, low: d.low, close: d.close, volume: d.volume, time: d.time })));
            indicatorsRef.current.vwap.setData(toSeriesData(vwapData));
        }
        else {
            if (indicatorsRef.current.vwap) {
                chart.removeSeries(indicatorsRef.current.vwap);
                delete indicatorsRef.current.vwap;
            }
        }
        ...
        if (indicatorsState.rsi14) {
            ...
            if (indicatorsRef.current.rsi) {
                chart.removeSeries(indicatorsRef.current.rsi);
                delete indicatorsRef.current.rsi;
            }
        }
        if (indicatorsState.macd) {
            ...
            if (indicatorsRef.current.macdLine) {
                ...
                delete indicatorsRef.current.macdLine;
                delete indicatorsRef.current.macdSignal;
                delete indicatorsRef.current.macdHist;
            }
        }
        // Subscribe to real-time updates (Simplistic update, just updating the main chart for now to save performance)
        wsClient.subscribe(activeSymbol, timeframe, (tick) => {
            if (mainSeriesRef.current && volumeSeriesRef.current) {
                mainSeriesRef.current.update({
                    time: tick.time, open: tick.open, high: tick.high, low: tick.low, close: tick.close, value: tick.close
                });
                volumeSeriesRef.current.update({
                    time: tick.time, value: tick.volume, color: tick.close >= tick.open ? upColor : downColor
                });
            }
        });
        return () => {
    ...
    return (<div className="w-full h-full relative flex-1">
      <PriceDisplay />
      <div className="w-full h-full absolute inset-0" ref={chartContainerRef}></div>
      {isLoading && (<div className="absolute inset-0 flex items-center justify-center bg-bg-primary/50 backdrop-blur-sm z-10">
