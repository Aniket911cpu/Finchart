...
export class WatchlistController {
  ...
  static async getWatchlists(req: FastifyRequest, reply: FastifyReply) {
  ...
  static async createWatchlist(req: FastifyRequest<{ Body: { name: string } }>, reply: FastifyReply) {
    const userId = (req.user as any)?.sub;
    if (!userId) return reply.code(401).send({ error: "Unauthorized" });
  ...
  static async addSymbol(req: FastifyRequest<{ Params: { id: string }, Body: { symbol: string, exchange?: string } }>, reply: FastifyReply) {
    const userId = (req.user as any)?.sub;
    if (!userId) return reply.code(401).send({ error: "Unauthorized" });
  ...
  static async deleteSymbol(req: FastifyRequest<{ Params: { id: string, symbolId: string } }>, reply: FastifyReply) {
    const userId = (req.user as any)?.sub;
    if (!userId) return reply.code(401).send({ error: "Unauthorized" });
