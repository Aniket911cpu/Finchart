...
export class UdfController {
    ...
    static async time(req, reply) {
        return Math.floor(Date.now() / 1000);
    }
    static async symbols(req, reply) {
        const { symbol } = req.query;
        if (!symbol)
            return reply.code(400).send("Symbol required");
    ...
    static async history(req, reply) {
        const { symbol, resolution, from, to } = req.query;
        if (!symbol || !resolution || !from || !to) {
            return reply.code(400).send({ s: "error", errmsg: "Missing parameters" });
        }
        const data = await marketService.getHistory(symbol, resolution, parseInt(from, 10), parseInt(to, 10));
        if (data.length === 0) {
            return { s: "no_data" };
        }
        return {
...
//# sourceMappingURL=udf.controller.js.map