...
export const getUserSettings = async (req, reply) => {
...
export const updateUserSettings = async (req, reply) => {
    const userId = req.user?.sub;
    if (!userId)
        return reply.status(401).send({ error: 'Unauthorized' });
    const data = req.body;
...
export const getApiKeys = async (req, reply) => {
    const userId = req.user?.sub;
    if (!userId)
        return reply.status(401).send({ error: 'Unauthorized' });
...
export const createApiKey = async (req, reply) => {
    const userId = req.user?.sub;
    if (!userId)
        return reply.status(401).send({ error: 'Unauthorized' });
...
export const revokeApiKey = async (req, reply) => {
    const userId = req.user?.sub;
    if (!userId)
        return reply.status(401).send({ error: 'Unauthorized' });
